...
export const Carousel = ({ slides, autoplay }) => {
	...
	const SliderButton = ({ children, classes, onClick }) => {
		const classNames = `absolute flex items-center p-2 sm:p-4 ${classes} bottom-0 sm:bottom sm:bottom-45% sm:rounded-full bg-bg bg-opacity-0 sm:bg-opacity-50 sm:hover:bg-opacity-80 z-10 cursor-pointer select-none backdrop-filter sm:backdrop-blur`;
		return (
